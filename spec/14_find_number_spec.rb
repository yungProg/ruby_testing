...
describe FindNumber do
  ...
  describe '#game_over?' do
    context 'when guess and random_number are equal' do
      ...
      let(:correct_guessing) { double('correct_guessing') }
      subject(:game_guessing) { described_class.new(0, 9, correct_guessing, 7) }
      ...
      it 'is game over' do
        allow(correct_guessing).to receive(:value).and_return(7)
    ...
    context 'when guess and random_number are not equal' do
      ...
      it 'is not game over' do
        allow(correct_guessing).to receive(:value).and_return 3
  ...
  describe '#update_range' do
    ...
    context 'when the guess is less than the answer' do
      ...
      before do
        min_update = low_guess_game.update_range
      end
    ...
    context 'when the guess is more than the answer' do
      ...
      before do
        min_update = high_guess_game.update_range
      end
    ...
    context 'when the guess is 7, min=5, and max=8' do
      let(:edge_case) { double('random_number', value: 8) }
      subject(:game_guessing) { described_class.new(5, 8, edge_case, 7) }

      before do
        guess_update = game_guessing.update_range
      end
