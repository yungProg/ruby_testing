...
describe Cat do
  ...
  subject(:pyne) { described_class.new('Pyne')}
  # Write a test using the second shared_example to test that cat responds to
  # talk ('meow').
  context 'when method is shared among various classes' do
    it 'is expected to respond to talk' do
      expect(pyne).to  respond_to(:talk)
    end
  end

  # remove the 'x' before running this test
  it 'is not hungry' do
    expect(pyne).not_to be_hungry
  end

  # remove the 'x' before running this test
  it 'is hiding' do
    expect(pyne).to be_hiding
  end
end
